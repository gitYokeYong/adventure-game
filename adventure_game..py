...
def print_with_sleep(text, sleep_time=1):
    print(text)
...
def encounter_enemy(enemy):
    ...
    while True:
        response = input("Enter 1 to knock on the door of the house, Enter 2 go to the cave instead: ")

        if response == "1":
            handle_house_encounter(found_sword, enemy)
        elif response == "2":
            if not found_sword:
                #found_sword= handle_cave_encounter()
                print_with_sleep("You decided to check out the dark cave with bravery.")
                print_with_sleep("You peer cautiously into the cave")
                # Add the following line to return to the empty_cave_message
                empty_cave_message()
                return encounter_enemy(enemy)
        else:
            print_with_sleep("Please enter 1 or 2")

def handle_house_encounter(found_sword, enemy):
    if not found_sword:
        ...
        if fight_or_run == "1":
            ...
            if response == "1":
                return  # Exit the loop and go back to the field
            elif response == "2":
                found_sword= handle_cave_encounter()
    else:
        print_with_sleep("You approach the door of the house.")
        print_with_sleep(f"As you knock, the door opens, and out steps a {enemy}")
        print_with_sleep("Eep! It looked scarier than your first encounter!")
        print_with_sleep(f"The {enemy} attacks you")
